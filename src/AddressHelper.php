...
class AddressHelper
{
    ...
    public static function standardizePostcodeNL(string $postcode)
    {
        if (!preg_match('/^(\d{4})(| )(\w{2})$/', trim($postcode), $matches)) {
            throw new \Exception('Not a valid postcode provided', 1);
        }

        return $matches[1] . strtoupper($matches[3]);
    }

    public static function postcodeToArray(string $postcode)
    {
        return preg_split('/\d{4} \w{2}/', $postcode);
    }
}
