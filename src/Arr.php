...
class Arr
{
    public const FLAT_QUERY_EXPRESSION_OPENER = '/^';
    public const FLAT_QUERY_EXPRESSION_CLOSER = '.*$/';

    public const FLAT_QUERY_EXPRESSION_WILDCARD = '(.*)';

    public const FLAT_QUERY_EXPRESSION_STRING_OPEN = '(';
    public const FLAT_QUERY_EXPRESSION_STRING_CLOSE = ')';

    public const FLAT_QUERY_EXPRESSION_NUMERIC_OPEN = '(\[';
    public const FLAT_QUERY_EXPRESSION_NUMERIC_CLOSE = '\])';

    public const FLAT_QUERY_EXPRESSION_SEPARATOR = '\.';

    public function issetAnyOf(array $haystack, array $needles) : bool
    {
    ...
    public function issetAllOf(array $haystack, array $needles) : bool
    {
        $results = [];
        foreach($needles as $needle) {
            $results = isset($haystack[$needle]);
        }
    ...
    /**
     * Reference alias of Blackdoor\Util\ArrayHelper::sanitize()
     *
     * @param mixed $data           The to be sanitized resource
     * @param boolean $array        Should it return an array? (if false, will return a StdClass object)
     ...
     */
    public static function keysAreSet(array $keys, array $array, &$missing, bool $requireAValue = false): bool
    {
        $missing = [];
        ...
        foreach ($keys as $key) {
            ...
            if ($requireAValue && array_key_exists($key, $array) && empty($array[$key])) {
                $missing[] = $key;
            }
        }
    ...
    public static function search($value, array $array, bool $strict = false)
    {
        ...
        $found = array_search($value, $array, $strict);
     ...
     */
    public static function searchByQuery(string $query, array $array, &$found = null, $strict = false): bool
    {
        if (self::isMultidimensional($array)) {
            $array = self::flatten($array);
        }
    ...
    public static function contains($needles, array $haystack)
    {
        if (!is_array($needles)) {
            $needles = [$needles];
        }
    ...
    public static function containsAny($needles, array $haystack)
    {
        if (!is_array($needles)) {
            $needles = [$needles];
        }
     ...
     * @param bool $strict
     */
    public static function requiredKeysAreSet(array $parameters, array $data, &$missing, bool $strict = false): bool
    {
        $missing = [];
        $isAssoc = false;
    ...
    public static function allKeysAreFilled(array $data, &$empty)
    {
        $empty = [];
     ...
     * @return mixed
     */
    public static function flatten(array $input, $parent = null): array
    {
        $array = [];
     ...
     * @param array $map
     */
    public static function rebuildArray(array $map, bool $skipCleanup = false)
    {
        if (empty($map)) {
            return [];
        }
        ...
        return $skipCleanup ? $array : self::cleanupRebuild($array);
    }
     ...
     *
     *      Add '.' separators for multidimensional arrays
     *      'Email.primary' => ['field' => 'email'],
     *
     *      Use numbers for list arrays
    ...
    public static function combine(array $keys, array $values, $padValue = null): array
    {
        $length = max(count($keys), count($values));
        return array_combine(array_pad($keys, $length, $padValue), array_pad($values, $length, $padValue));
    }
}
