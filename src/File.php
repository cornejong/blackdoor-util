...
use SouthCoast\KickStart\Validate;
use RegexIterator;
use RecursiveIteratorIterator;
use RecursiveDirectoryIterator;
use Blackdoor\Utilor\FileError;

class File
{
    public const BASE_DIRECTORY = 'base_directory';
    public const DIRECTORY_MAP_IDENTIFIER = '$';
    public const NOTHING = '';

    public const Minified = 'minify_contents';
     ...
     * @throws FileError
     */
    public static function list(string $path, string $extension = null, bool $files_only = true, bool $subtract_base_path = true): array
    {
     ...
     * @throws FileError
     */
    public static function getPath(string $path, bool $needToExist = false): string
    {
        ...
        if (!Validate::path($path) && $needToExist) {
            throw new FileError(FileError::NOT_VALID_PATH, $path);
        }
     ...
     */
    public static function recursiveList(string $directory, string $pattern = '/.*$/'): array
    {
        /* Get the real path to the directory */
        $path = self::getPath($directory);

        /* Create a new Recursive directory object */
        $recursive_directory = new RecursiveDirectoryIterator($path, RecursiveDirectoryIterator::SKIP_DOTS);
     ...
     */
    public static function getJson(string $path, bool $returnObject = false)
    {
        return Json::parse(self::get($path, true), ($returnObject) ? false : true);
    }
    ...
    public static function parseCsvLine(string $line, string $delimiter = ',', string $enclosure = null): array
    {
        ...
        foreach ($line_array as &$value) {
            if ($enclosure !== null || $enclosure !== '') { /* Trim the enclosure  */
     ...
     */
    public static function makeDirectory(string $path, string $directory)
    {
        /* Get the real path to the original directory */
        $path = self::getPath($path, true);
        /* Format the new path */
    ...
    public static function store(string $path, $data)
    {
        /* Get the real path to the original directory */
        $path = self::getPath($path);
    ...
    public static function storeJson(string $path, $data, ...$options)
    {
        /* Get the real path to the original directory */
        $path = self::getPath($path);

        $data = in_array(File::Minified, $options) ? Json::encode($data) : Json::prettyEncode($data);

        return File::store($path, $data);
    }


    public static function getGenerator(string $file)
    {
        $handle = fopen($file, 'r');
    ...
    public static function directoryGenerator(string $directory, bool $skipDot = true)
    {
        ...
        while (($filename = readdir($handle)) !== false) {
            ...
            yield $filename => ($directory . DIRECTORY_SEPARATOR . $filename);
        }

        closedir($handle);
    }

    public static function getStream(string $file, $mode = 'r')
    {
