...
class Number
{
     ...
     *
     * @param numeric $number_1 The first number.
     * @param numeric $number_2 The second number.
     * @return string A string containing the ratio.
     */
    public static function getRatio($number_1, $number_2): string
    {
        if (!\is_numeric($number_1) || !\is_numeric($number_2)) {
            throw new \InvalidArgumentException("Arguments van only be numeric values!", 1);
        }

        for ($n = $number_2; $n > 1; $n--) {
    ...
    /**
     * Calculate the percentage of a number within a total
     *
     * @param numeric $total
    ...
    {
        if (!\is_numeric($total) || !\is_numeric($number)) {
            throw new \InvalidArgumentException("Arguments van only be numeric values!", 1);
        }

        return $total > 0 ? round($number / ($total / 100), 2) : 0;
    }
}
